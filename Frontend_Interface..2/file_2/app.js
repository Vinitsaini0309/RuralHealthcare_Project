/* ==========================================================================
   ...
   + Direct Redirection for Warehouse Owner Dashboard & Clinic Dashboard
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const roleConfigs = {
    doctor: {
      banner: 'Authenticating for St. Jude Clinical Staff & Physicians',
      placeholder: 'e.g. DOC-9942@medico.org'
    },
    pharmacy: {
      banner: 'Authenticating for Warehouse Owner & Depot Managers',
  ...
  roleTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      currentActiveRole = tab.dataset.role;
      if (roleConfigs[currentActiveRole]) {
        roleBannerText.textContent = roleConfigs[currentActiveRole].banner;
        usernameInput.placeholder = roleConfigs[currentActiveRole].placeholder;
      }
    });
  });
  ...
  /* ------------------------------------------------------------------------
     5. DEMO LOGIN AUTOFILL HELPERS & REDIRECTION
     ------------------------------------------------------------------------ */
  ...
  if (demoDoctorBtn) {
    demoDoctorBtn.addEventListener('click', () => {
      ...
      // Fast redirect to Clinic Dashboard
      setTimeout(() => {
  ...
  if (demoPharmacyBtn) {
    demoPharmacyBtn.addEventListener('click', () => {
      ...
      usernameInput.value = 'warehouse.owner@medico-depot.com';
      passwordInput.value = 'WarehouseOwner#2026';
      passwordInput.dispatchEvent(new Event('input'));
      
      // Fast redirect to Warehouse Owner Dashboard
      setTimeout(() => {
  ...
  /* ------------------------------------------------------------------------
     7. FORM SUBMISSION & SUCCESS MODAL / REDIRECT TO DASHBOARDS
     ------------------------------------------------------------------------ */
  ...
  if (loginForm && submitBtn && successModal) {
    loginForm.addEventListener('submit', (e) => {
      ...
      setTimeout(() => {
        ...
        if (currentActiveRole === 'pharmacy') {
          modalSuccessText.textContent = `Welcome, Warehouse Owner (${userVal}). Redirecting to Medico Depot Control Center...`;
          successModal.classList.add('active');

          setTimeout(() => {
            window.location.href = 'warehouse-dashboard.html';
          }, 1200);
        } else if (currentActiveRole === 'doctor') {
          modalSuccessText.textContent = `Welcome, Dr. Sarah Jenkins. Redirecting to St. Jude Clinic Dashboard...`;
          successModal.classList.add('active');
